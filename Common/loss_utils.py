...
"""CD là một phép đo khoảng cách đối xứng, tính trung bình khoảng cách từ mỗi điểm
trong một đám mây điểm này đến điểm gần nhất trong đám mây điểm còn lại, và ngược lại.

EMD là một phép đo khoảng cách, tính chi phí tối thiểu để biến đổi một đám mây điểm 
thành đám mây điểm còn lại, xem các điểm như các phân phối trong không gian. 
Phương pháp này thường được sử dụng trong các ứng dụng như 
so khớp hình dạng (shape matching) và đăng ký đám mây điểm (point cloud registration). 
Hàm mất mát EMD được tính bằng cách tìm phép ghép tối ưu giữa các điểm trong 
hai đám mây điểm, sao cho tổng khoảng cách giữa các cặp điểm được ghép là nhỏ nhất.
...
def py_uniform_loss(points,idx,pts_cn,radius):
    ...
    for i in range(B):
        ...
        for j in range(npoint):
            ...
            if disk_point.shape[0]<0:    #dòng này có vấn đề, điều kiện này gần như không bao giờ xảy ra vì đã có dòng 73
...
def get_uniform_loss(pcd, percentages=[0.004,0.006,0.008,0.010,0.012], radius=1.0):
    ...
    loss = []
    for p in percentages:
        nsample = max(2, int(N * p))  # Đảm bảo nsample >= 2
        r = math.sqrt(p * radius)
        ...
        grouped_pcd = group_point(pcd, idx)  # (B, npoint, nsample, 3)
        grouped_pcd = tf.concat(tf.unstack(grouped_pcd, axis=1), axis=0)  # (B*npoint, nsample, 3)

        # Nếu patch chỉ còn 1 điểm, pad thêm điểm giả để tránh lỗi
        shape = tf.shape(grouped_pcd)
        nsample_actual = shape[1]
        def pad_grouped_pcd():
            pad_width = [[0, 0], [0, 2 - nsample_actual], [0, 0]]
            return tf.pad(grouped_pcd, pad_width)
        grouped_pcd_safe = tf.cond(nsample_actual < 2, pad_grouped_pcd, lambda: grouped_pcd)

        var, _ = knn_point(2, grouped_pcd_safe, grouped_pcd_safe)
        uniform_dis = -var[:, :, 1:]
...
#định nghĩa thêm hàm mất mát cho đường thẳng PCA
# Hàm này tính toán khoảng cách bình phương trung bình từ các điểm đến đường thẳng
def line_fit_loss(points):
    """
    points: (batch_size, N, 3)
    Loss là khoảng cách bình phương trung bình từ các điểm đến đường thẳng PCA tốt nhất.
    """
    import tensorflow as tf

    mean = tf.reduce_mean(points, axis=1, keepdims=True)  # (B, 1, 3)
    centered = points - mean  # (B, N, 3)

    # SVD: lấy vector chính đầu tiên (hướng của dây)
    s, u, v = tf.linalg.svd(centered)
    direction = tf.expand_dims(v[:, :, 0], axis=1)  # (B, 1, 3)

    # vector từ mean đến điểm
    vec = centered  # (B, N, 3)

    # chiếu vec lên direction
    proj = tf.reduce_sum(vec * direction, axis=2, keepdims=True) * direction  # (B, N, 3)

    # Khoảng cách từ điểm đến đường thẳng = norm(vec - proj)
    dist = tf.norm(vec - proj, axis=2)  # (B, N)
