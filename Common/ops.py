...
from tensorflow.keras.layers import Conv2D as conv2d_kr
import numpy as np
import os
import sys
sys.path.append(os.path.dirname(os.getcwd()))
def mlp(inputs, layer_units, normalizer_fn=None, normalizer_params=None, activation_fn=None, scope=None):
    x = inputs
    for i, units in enumerate(layer_units):
        x = tf.keras.layers.Dense(
            units=units,
            activation=None,  # Không truyền activation ở đây nếu dùng batchnorm
            name=f"{scope}_fc{i}" if scope else None
        )(x)
        if normalizer_fn is not None:
            if normalizer_params is None:
                normalizer_params = {}
            x = normalizer_fn(**normalizer_params)(x)
        if activation_fn is not None:
            x = activation_fn(x)
    return x

def conv2d_keras(inputs, filters, kernel_size, normalizer_fn=None, normalizer_params=None, activation_fn=None, scope=None):
    x = conv2d_kr(filters, kernel_size, padding='valid', name=scope, use_bias=False)(inputs)
    if normalizer_fn is not None:
        if normalizer_params is None:
            normalizer_params = {}
        x = normalizer_fn(**normalizer_params)(x)
...
def tf_covariance(data):
    ...
    mean_data = tf.reduce_mean(data, axis=2, keepdims=True)  # (batch_size, num_point, 1, 3)
