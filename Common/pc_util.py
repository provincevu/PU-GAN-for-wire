...
def group_wires_general(pc, tol=1e-3):
    """
    pc: (N, 3) array
    tol: tolerance for grouping (avoid floating point noise)
    Return: dict {wire_identity: np.array([M, 3])}
    """
    # Bước 1: xác định trục kéo dài nhất
    variances = np.var(pc, axis=0)
    main_axis = np.argmax(variances)
    other_axes = [i for i in range(3) if i != main_axis]

    # Bước 2: group theo hai trục còn lại
    wires = {}
    for point in pc:
        k1 = round(point[other_axes[0]] / tol) * tol
        k2 = round(point[other_axes[1]] / tol) * tol
        key = (other_axes[0], k1, other_axes[1], k2)  # key chứa trục và giá trị
        if key not in wires:
            wires[key] = []
        wires[key].append(point)
    for key in wires:
        wires[key] = np.array(wires[key])
    return wires, main_axis

# def extract_segments_per_wire_general(wires, main_axis, patch_length=32):
#     """
#     wires: dict {wire_identity: np.array([M, 3])}
#     main_axis: int (0, 1, 2)
#     """
#     all_patches = []
#     for key, wire in wires.items():
#         # sort theo trục chính
#         wire_sorted = wire[np.argsort(wire[:, main_axis])]
#         for start in range(0, len(wire_sorted) - patch_length + 1, patch_length):
#             patch = wire_sorted[start:start+patch_length]
#             all_patches.append(patch)
#     return np.stack(all_patches) if all_patches else None
...
def extract_segments_per_wire_general(wires, main_axis, patch_length=32):
    ...
    for key, wire in wires.items():
        ...
        wire_sorted = wire[np.argsort(wire[:, main_axis])]
        L = len(wire_sorted)
        for start in range(0, L, patch_length):
            patch = wire_sorted[start:start+patch_length]
            if patch.shape[0] == patch_length:
                all_patches.append(patch)
            elif patch.shape[0] >= 3:
                # Pad patch cuối bằng random sample từ chính patch đó
                pad_size = patch_length - patch.shape[0]
                idxs = np.random.choice(patch.shape[0], pad_size, replace=True)
                patch_pad = np.concatenate([patch, patch[idxs]], axis=0)
                all_patches.append(patch_pad)
            # nếu patch quá nhỏ (<3 điểm) thì bỏ qua
    return np.stack(all_patches) if all_patches else None
